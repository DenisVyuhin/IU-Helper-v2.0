   ...
   if default_tags:
      try:
         normal_hashtags = await mistral_ai(
            f"входящие хештеги: {default_tags} Эти хештеги преобразуй в формат: #предмет #<число>неделя #<число>год #<число>класс. Предметы ТОЛЬКО из списка: {con.school_subject}. Если изначально год отсутствует – не добавляй его. В качестве ответа дай только 3 (если немецкий или китайский, то 4) хештега, без лишнего текста."
         )
      except Exception as e:
